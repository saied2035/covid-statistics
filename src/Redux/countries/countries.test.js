...
import countriesReducer, { filerCountries, fetchCountries } from './countries';
  ...
  test('fetchCountries test', () => {
    ...
    return store.dispatch(fetchCountries())
      .then(() => {
        ...
        expect(actions[0].type).toEqual('covid-statistics/countries/COUNTRY_LOAD');
        expect(actions[1].type).toEqual('covid-statistics/countries/COUNTRY_LOADED');
      });
  });
  test('filerCountries test', () => {
    const store = mockStore({});
    store.dispatch(filerCountries());
    const actions = store.getActions();
    expect(actions[0].type).toEqual('covid-statistics/search/FILTER_COUNTRIES');
  });
});
...
describe('countriesReducer', () => {
  ...
  test('test countriesReducer wait countries', () => {
    ...
    const action = {
      type: 'covid-statistics/countries/COUNTRY_LOAD',
    };
  ...
  test('test countriesReducer countries loaded', () => {
    ...
    const action = {
      type: 'covid-statistics/countries/COUNTRY_LOADED',
  ...
  test('test countriesReducer countries load failed', () => {
    ...
    const action = {
      type: 'covid-statistics/countries/COUNTRY_LOAD_FAILED',
